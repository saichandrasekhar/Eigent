...
import { HttpMcpProxy } from "./http-proxy.js";
...
// ── Proxy command (HTTP/SSE transport) ──────────────────────────────────

program
  .command("proxy")
  .description("Start an HTTP reverse proxy for Streamable HTTP MCP servers")
  .requiredOption(
    "--upstream <url>",
  ...
  .option(
    "--policy <path>",
    "Path to a policy file (reserved for future use)",
  )
  .allowUnknownOption(false)
  .action(async (options: {
    ...
    if (isNaN(port) || port < 1 || port > 65535) {
      process.stderr.write(
        `[eigent-sidecar] Invalid port '${options.port}'. Must be 1-65535.\n`,
      );
      process.exit(1);
    }
    ...
    const handleSignal = (signal: string) => {
      ...
      stopping = true;
      process.stderr.write(
        `[eigent-sidecar] Received ${signal}, shutting down proxy...\n`,
      );
      proxy.stop().then(() => {
        process.exit(0);
      }).catch(() => {
